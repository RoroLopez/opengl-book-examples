...
fn main() {
    ...
    // Shader setup
    let vertex_shader_id = match Shader::load_shader(ShaderType::Vertex, Path::new("src/shaders/vertex/light_colors.vert")) {
        Ok(id) => id,
        Err(e) => {
            panic!("{}", e.to_string())
        }
    };
    let cube_shader_id = match Shader::load_shader(ShaderType::Fragment, Path::new("src/shaders/fragment/light_colors.frag")) {
        Ok(id) => id,
        Err(e) => {
            panic!("{}", e.to_string())
        }
    ...
    match light_source_shader_program.build(&[vertex_shader_id, light_source_shader_id]) {
        Err(e) => {
            panic!("{}", e.to_string())
        },
        _ => {}
    ...
    while !window.should_close() {
        ...
        unsafe {
            ...
            shader_program.set_vec3(c"lightColor", (1.0, 1.0, 1.0));
